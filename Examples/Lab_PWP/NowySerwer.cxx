...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  igtl::PointMessage::Pointer msgToSend;
  msgToSend = igtl::PointMessage::New();
  msgToSend->SetDeviceName("PointSender");
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
      ...
      newPoint->SetRGBA(rgba[0], rgba[1], rgba[2], rgba[3]);
      newPoint->SetPosition(-pos[0], -pos[1], -pos[2]);
      newPoint->SetRadius(15.0);
      newPoint->SetOwner(pointElement->GetOwner());

      msgToSend->AddPointElement(newPoint);
    }
  }

  msgToSend->Pack();
    // Send back
  socket->Send(msgToSend->GetPackPointer(), msgToSend->GetPackSize());
  std::cerr << "Packet Sent back" << std::endl;
  return 1;
}
